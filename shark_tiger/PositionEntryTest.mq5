...
NewCandleDetector* newCandleDetectorTest ;
...
input datetime zoneLeftEdge ;
input double zoneHighEdge;
input double zoneLowEdge;
input double stopLosssPrice;
input double takeProfittPrice ;
input double takeProfittPrice2;
...
//const int UrlDefinedErrorMt4 = 4066 ; // this url defined error is for MT4
//const int    UrlDefinedError  = 4014; // Because MT4 and MT5 are different




// VARIABLES FOR TELEGRAM PUBLISHER
//string fileType ;
//string fileName;
...
//--- create timer
   ...
   objectsManager.drawRectangleInStrategyTester(1,iTime(Symbol(),PERIOD_CURRENT,130),iTime(Symbol(),PERIOD_CURRENT,0),zoneHighEdge,zoneLowEdge);
   zoneContainer.incrementZonesIdCounter();
   Zone* newZone = new Zone() ;
   newZone.setHigherEdgePrice(zoneHighEdge);
   newZone.setLowerEdgePrice(zoneLowEdge);
   newZone.setLeftEdge(iTime(Symbol(),PERIOD_CURRENT,130));
   newZone.setRightEdge(iTime(Symbol(),PERIOD_CURRENT,0));
   newZone.setId(objectConfUnit.getConfirmationObjectId());
   newZone.setTimeFrame(getChartTimeFrameInString());
   ...
   if(zoneHighEdge < ask)   // means its a support zone
     {
   ...
   if(zoneLowEdge >  bid)  // means its a resistance zone
     {
...
// add the zone to the data structure
   ...
   newZone.setStopLossPrice(stopLosssPrice);
   newZone.setTakeProfit(takeProfittPrice);
   newZone.setTakeProfit(takeProfittPrice2);

   newZone.printTradeManagerData();
