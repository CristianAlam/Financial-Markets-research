   ...
   if(resistancePatternFormed(timeFrame))   // if resistance formed
     {
      ...
      double _higherEdgePrice  = resistancePrice + resistanceExtendAboveCandle;
      double _lowerEdgePrice = resistancePrice - resistanceLowerEdgeExtend ;
               ...
               if(result == 1)
                 {
                  ...
                  string currentIdCounter = IntegerToString(zoneContainer.getZonesIdCounter());
                  Zone* newZone = new Zone(currentIdCounter,_higherEdgePrice,_lowerEdgePrice,_leftEdge,_rightEdge,timeFrameStr,"TYPE_RESISTANCE_BREAKOUT") ;
                  zoneContainer.addResistanceZoneTiger(newZone);
                  zoneContainer.incrementZonesIdCounter();
                  objectsManager.drawRectangleInStrategyTester(currentIdCounter,_leftEdge,_rightEdge,resistancePrice + resistanceExtendAboveCandle,resistancePrice - resistanceLowerEdgeExtend,BOS_zone_color);
                  ...
                  if(result == 0)
                    {
                     ...
                     Zone* newZone = new Zone(currentIdCounter,_higherEdgePrice,_lowerEdgePrice,_leftEdge,_rightEdge,timeFrameStr,"TYPE_RESISTANCE_NORMAL") ;
   ...
   if(supportPatternFormed(timeFrame))   // if support formed
     {
      ...
      double _higherEdgePrice  = supportPrice + supportHigherEdgeExtend;
      double _lowerEdgePrice = supportPrice - supportExtendBelowCandle;
      string _zoneId = IntegerToString(currentIdCounter);
      ...
      if((zoneContainer.getNumberOfActiveZones() != 0) && (supportPrice - zoneContainer.getZoneByIndex(zoneContainer.getNumberOfActiveZones()-1).getHigherEdge() >= rangeDistanceBetweenZones))   // check if it has a clean range down
        {
         ...
         Zone* newZone = new Zone(currentIdCounter,_higherEdgePrice,_lowerEdgePrice,_leftEdge,_rightEdge,timeFrameStr,"TYPE_SUPPORT_BREAKOUT") ;
         ...
         if((zoneContainer.getNumberOfActiveZones() != 0) && (supportPrice - zoneContainer.getZoneByIndex(zoneContainer.getNumberOfActiveZones()-1).getHigherEdge() < rangeDistanceBetweenZones))   // if it doesnt have a clean range, just add it as a blue zone
           {
            Zone* newZone = new Zone(currentIdCounter,_higherEdgePrice,_lowerEdgePrice,_leftEdge,_rightEdge,timeFrameStr,"TYPE_SUPPORT_NORMAL") ;
            zoneContainer.addSupportZoneTiger(newZone);
            zoneContainer.incrementZonesIdCounterSupport();
            ...
            if(zoneContainer.getNumberOfActiveZones() == 0)   // this means this is the first zone to add to the data strucutre
              {
               ...
               if(result == 1)   // means if found a lower zone and the range is clean
                 {
                  string currentIdCounter = IntegerToString(zoneContainer.getSupportZonesIdCounter());
                  Zone* newZone = new Zone(currentIdCounter,_higherEdgePrice,_lowerEdgePrice,_leftEdge,_rightEdge,timeFrameStr,"TYPE_SUPPORT_BREAKOUT") ;
                  zoneContainer.addSupportZoneTiger(newZone);
                  zoneContainer.incrementZonesIdCounterSupport();
         ...
         if((resistancePrice > currentHigherResistancePrice) && ((resistancePrice - currentHigherResistancePrice) > rangeDistanceBetweenZones))   // this means the range is valid
           {
            ...
            if((resistancePrice > currentHigherResistancePrice) && !((resistancePrice - currentHigherResistancePrice) > rangeDistanceBetweenZones))   // the range is not valid, this means draw only the old one
              {
         ...
         if((supportPrice < currentLowerSupportPrice) && ((currentLowerSupportPrice - supportPrice) > rangeDistanceBetweenZones))   // this means the range is valid
           {
            ...
            if((supportPrice < currentLowerSupportPrice)  && !((currentLowerSupportPrice - supportPrice) > rangeDistanceBetweenZones))   // the range is not valid, this means draw only the old one
              {
